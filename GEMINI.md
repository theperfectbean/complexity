...
- **Sidebar Cleanup**: Removed the "Thread Branches" section from the Sidebar as it was redundant and visually cluttered.
- **Assistant Message Footer**: Consistently grouped response-level actions (Copy, Export, Retry, Rewrite) in the message footer.
- **Share Thread**: (Removed 2026-03-22) The "Share" button that copied the thread URL was removed as it was not required.

### Thread Title Summarization (2026-03-21)
- **Problem**: Thread titles were generated by simple truncation of the user's first message, leading to poor visual quality in the sidebar and recent chats list.
- **Implementation**:
  - Added `generateThreadTitle` utility in `app/src/lib/llm.ts` using the Vercel AI SDK `generateText` method.
  - The utility uses a "fast" model (e.g., Claude Haiku or Sonar) to summarize the initial user query into a concise, high-quality title (3-6 words).
  - Updated `POST /api/threads` to make the `title` field optional and accept an `initialMessage`. If no title is provided, the backend automatically generates one from the message.
  - Refined `resolveRequestedModel` to ensure it always returns a model compatible with standard Chat completions (non-presets) when requested for background tasks like titling.
  - Updated frontend entry points (`page.tsx` and Role detail page) to pass the full query as `initialMessage`.
- **Benefit**: Conversations now have descriptive, readable titles that improve navigation and platform aesthetics.
