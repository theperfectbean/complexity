...
- **Input Box Jumping**: During streaming, the fixed input box would jump up and down. This was caused by two factors: (1) using `scrollIntoView` on an element under a fixed footer, which forces the browser viewport to shift awkwardly. This was replaced with a direct `window.scrollTo` wrapped in a `requestAnimationFrame` to ensure it synchronizes perfectly with the browser's paint cycle. (2) Framer Motion's `layout` prop continuously recalculating bounding boxes while the window was programmatically scrolling; the `layout` prop on the `SearchBar` is now strictly disabled (`layout={!compact}`) whenever the search bar is rendered at the bottom of the chat interface.
