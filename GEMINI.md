...
### UI & UX Improvements
- **SearchBar Attachment Button**: The "Attach file" button in the `SearchBar` was previously non-functional. It has been updated to trigger a hidden `<input type="file" multiple />` when clicked. The `onAttachClick` prop was implemented in both the `Home` page and the `ThreadChat` page to provide immediate feedback via toast notifications ("File attachments coming soon").
