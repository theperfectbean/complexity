...
- **Body Size Limit**: Increased Next.js middleware body size limit to 20MB in `next.config.ts` using `experimental.middlewareClientMaxBodySize`. This resolved 'TypeError: Failed to parse body as FormData' errors when uploading large files (>10MB).
