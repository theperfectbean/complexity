...
export default function Home() {
  ...
  async function startThread(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/chat/${payload.thread.id}?q=${encodeURIComponent(query.trim())}&web=${webSearchEnabled}`);
    } catch (error) {
      setLoading(false);
      ...
      <div className="flex flex-1 items-center justify-center">
        <form onSubmit={startThread} className="w-full max-w-3xl space-y-3">
          <div className="mb-8 space-y-2 text-center">
            ...
            <p className="text-lg text-muted-foreground">Your intelligent AI assistant</p>
          </div>
          <SearchBar
            key="home-searchbar"
            id="home-searchbar"
