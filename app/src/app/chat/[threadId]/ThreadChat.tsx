...
function ShareButton({ threadId }: { threadId: string }) {
  ...
  const handleShare = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/threads/${threadId}/share`, { method: "POST" });
  ...
  return (
    <button
      ...
      className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-black/5 dark:hover:bg-white/5 active:scale-95 disabled:opacity-50"
    >
      {copied ? (
        <Check className="h-3.5 w-3.5 text-emerald-500" />
      ) : (
        <Share2 className="h-3.5 w-3.5 text-muted-foreground/60" strokeWidth={1.5} />
      )}
    </button>
  );
}
...
export function ThreadChat({
  ...
  const handleEditMessage = useCallback(
    async (messageId: string, newContent: string) => {
      ...
      router.push(`/chat/${newThreadId}?q=${encodeURIComponent(newContent)}&web=${webSearchEnabled}`);
    },
      ...
      <div className="flex-1 space-y-12">
        <MessageList
          ...
          onDownload={() => exportMessagesAsMarkdown(threadTitle, mergedMessages)}
              shareButton={<ShareButton threadId={threadId} />}
