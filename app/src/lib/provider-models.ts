...
export async function fetchProviderModelsWithStatus(): Promise<ProviderDiscoveryResult> {
  ...
  // We check if the search provider itself is enabled
  const isSearchEnabled = (searchProvider === "perplexity" && keys["PROVIDER_PERPLEXITY_ENABLED"] === "true") ||
                          (searchProvider === "tavily" && keys["SEARCH_API_KEY"] !== null); // Tavily is usually in SEARCH_API_KEY

  if (searchApiKey && searchProvider !== "none" && isSearchEnabled) {
  ...
  if (keys["PERPLEXITY_API_KEY"] && keys["PROVIDER_PERPLEXITY_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  // 2. Anthropic
  if (keys["ANTHROPIC_API_KEY"] && keys["PROVIDER_ANTHROPIC_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  // 3. OpenAI
  if (keys["OPENAI_API_KEY"] && keys["PROVIDER_OPENAI_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  // 4. Google Gemini
  if (keys["GOOGLE_GENERATIVE_AI_API_KEY"] && keys["PROVIDER_GOOGLE_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  // 5. xAI
  if (keys["XAI_API_KEY"] && keys["PROVIDER_XAI_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  if (ollamaUrl && keys["PROVIDER_OLLAMA_ENABLED"] === "true") {
    promises.push((async () => {
      try {
  ...
  // 7. Local OpenAI
  if (keys["LOCAL_OPENAI_BASE_URL"] && keys["PROVIDER_LOCAL_OPENAI_ENABLED"] === "true") {
    promises.push((async () => {
      try {
