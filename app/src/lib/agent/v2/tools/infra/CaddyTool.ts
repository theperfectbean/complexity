...
export async function caddy_list_routes(): Promise<object> {
  const r = await sshExec('node02', 'pct exec 101 -- cat /etc/caddy/Caddyfile 2>&1');
  return { caddyfile: capOutput(r.stdout, 8192) };
}

export async function caddy_reload(): Promise<object> {
  const r = await sshExec('node02', 'pct exec 101 -- systemctl reload caddy 2>&1');
...
export async function caddy_add_vhost(params: { domain: string; upstream: string }): Promise<object> {
  ...
  const r = await sshExec('node02', `pct exec 101 -- bash -c 'echo '${escaped}' >> /etc/caddy/Caddyfile && systemctl reload caddy' 2>&1`);
...
export async function caddy_remove_vhost(params: { domain: string }): Promise<object> {
  const r = await sshExec('node02', `pct exec 101 -- bash -c "cat /etc/caddy/Caddyfile" 2>&1`);
