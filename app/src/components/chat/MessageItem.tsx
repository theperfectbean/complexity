...
import { Check, Copy, RotateCcw, Globe, Search, Brain, Database, Pencil, ChevronLeft, ChevronRight, Download, Trash2 } from "lucide-react";
...
import type { ChatCitation, ChatThinkingPart, ChatMessageItem, ChatBranch } from "./MessageList";
...
export function StatusIcon({ name, active }: { name: string; active?: boolean }) {
            ...
            {isEditing ? (
              <div className="flex flex-col gap-2 rounded-2xl bg-muted/60 px-5 py-3.5">
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={cancelEdit} disabled={isSaving} className="rounded-lg px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted transition-colors disabled:opacity-50">
                    Cancel
                  </button>
                  <button
                ...
                {message.thinking.map(part => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isActive ? <div className="h-2 w-2 animate-pulse rounded-full bg-primary" /> : <StatusIcon name={part.toolName} />}
                      </div>
                      <div className="flex flex-col">
                        ...
                        {isActive && part.input && typeof part.input === "object" && "query" in (part.input as Record<string, unknown>) && typeof (part.input as Record<string, unknown>).query === "string" ? (
                          <span className="text-[11px] opacity-60 line-clamp-1">Searching for: {(part.input as Record<string, string>).query}</span>
                        ) : null}
                        {part.result && <span className="text-[11px] opacity-60">{part.result}</span>}
                      </div>
                    </motion.div>
                  );
                })}
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                >
                  <AnimatePresence mode="wait" initial={false}>
                    {copiedId === message.id ? (
                      <motion.div key="check" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.1 }}>
                        <Check className="h-4 w-4 text-emerald-500" strokeWidth={2} />
                      </motion.div>
                    ) : (
                      <motion.div key="copy" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ duration: 0.1 }}>
                          ...
                          <div key={category} className="py-1">
                            ...
                            {options.map(option => (
                              <DropdownMenu.Item
                                ...
                                className={cn("flex cursor-pointer items-center rounded-lg px-3 py-2 text-sm outline-none transition-colors hover:bg-accent hover:text-accent-foreground")}
                              >
